...
namespace cura {
	...
	void VolumeDecomposer::decompose(Mesh& mesh, bool first){
		log("number of mesh faces = %d\n", mesh.faces.size());
		// If this is the base mesh, initialize the graph with the mesh as the base parent
		if (first) {
		...
		// Stores entry points to the sub-meshes (overhangs)
		...
		// Initialize the comparison polygon and the slice currently being iterated on
	...
	int VolumeDecomposer::splitFaces(Mesh& mesh, int faceID, PolygonRef intersectingPoly, pair<Point3, Point3> splitPoints) {
		...
		while (true) {
			...
			if ((splitPointVertexIntersectionIndices.first >= 0) && (splitPointVertexIntersectionIndices.second >= 0)) { //case III or case IV
				...
				recentlyMadeFaces.push_back(newFaceID);
				processedFaceIndices.push_back(true);
				...
				recentlyMadeFaces.push_back(newFaceIDs.first);
				recentlyMadeFaces.push_back(newFaceIDs.second);
